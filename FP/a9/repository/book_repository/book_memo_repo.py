...
class BookMemoRepo():
    ...
    def add(self, book: Book):
        """
        The function adds a new book to the repository. If there already is a book with such id, it raises an error
        :param book: the book entity
        :return: None
        """
        if self.search_by_id(book.book_id) is not None:
    ...
    def get_book_by_id(self, search_book_id):
        for book_id in self._data:
            if search_book_id == book_id:
                return self._data[book_id]
    ...
    def list_of_authors(self):
        authors=[]
        for book_id in self._data:
            if self._data[book_id].author not in authors:
                authors.append(self._data[book_id].author)
        return authors
