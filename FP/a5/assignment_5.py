...
def calculate_modulus(nr):
    return math.sqrt((get_real(nr)**2)+(get_imaginary(nr)**2))
    ...
    for i in range(n):
        while True:
            try:
                ...
                imaginary=int(input("Imaginary part of the number: "))
                nr=create_number(real, imaginary)
                if nr is not None:
                    numbers.append(nr)
                    break
            except ValueError as ve:
                print("Real and imaginary parts should be integers. Try again!")
...
def print_dp_pb(l):
    ...
    print("The length of the subsequence is:", len(l))
...
def create_number(real, imaginary):
    if -100>real or real>100 or imaginary<-100 or imaginary>100:
        print('Real and Imaginary values must be between -100 and 100 but were given as ' + str(real) + ' and ' + str(imaginary)+'.Try again!')
        return None
