...
def time_jump_sort(type_case, number_list):
    if type_case == 6:
        x = -1
    elif type_case == 7:
    ...
    row = jump_search(number_list, number_list[x])
    end_js = timeit.default_timer() * 1000
    return end_js-start_js
...
def build_table(list_of_lists, type_case):
    ...
    for i in range(len(list_of_lists)):
        ...
        time_jump_sort(type_case, list_of_lists[i])
        #if type_case==6:
         #   x=-1
        #elif type_case==7:
        ...
        table.add_row([i, len(list_of_lists[i]), time_jump_sort(type_case, list_of_lists[i]), end_is-start_is, end_ss-start_ss])
    return table
