...
def jump_search(search_list: list, x: int):
    ...
    prev = 0
    while search_list[int(min(step, len(search_list)) - 1)] < x: #n^(1/2)
        prev = step
        step += int(math.sqrt(len(search_list)))
...
the current element is only compared with the right-most element of the sorted sublist. The time complexity
is O(n).
...
"""
For shell sort, the worst-case complexity happens when the list is reversed and it's complexity is O(n^2).
...
def start():
    ...
    while True:
        ...
        if choice=="1":
            ...
            initial_copy=copy.copy(list_of_lists)
